...
let AuthController = class AuthController {
    ...
    async register(registerDto) {
        if (registerDto.password != registerDto.confirmationPassword)
            throw new common_1.BadRequestException('Password and confirmation not match');
    ...
    async registerAdmin(req, registerDto) {
        if (req.user.role != role_enum_1.default.ADMIN)
            throw new common_1.UnauthorizedException('You are not authorized to add admin');
        const checkUser = await this.authService.checkUsername(registerDto.username);
        if (checkUser)
            throw new common_1.ConflictException('Username already taken');
        let pass = await bcrypt.hash(registerDto.password, 10);
